...
| 角色 | 命名示例 |
|------|----------|
| 旧版（上次已同步进站） | `MSc(BA) Teaching plan 2026-27_20260903.pdf` |
| 新版（本次 PO 邮件附件） | `MSc(BA) Teaching plan 2026-27_20260909.pdf` |
...
PO 的 Teaching Plan 通常用 **彩色** 标出相对上一版的改动。检查时：

1. **打开新旧两份 PDF**（同目录、日期后缀相邻的两份）。
2. **不要只信邮件正文**。邮件可能只点名部分课（如只写 7002 & 7003），PDF 里仍可能有其它色标（如 7004 Tutorial 列）。
3. **先确认本版用色**：多数版本用 **红色** 标新改动；`20260909` 起可见 **绿色 = 上一版已同步的旧改动残留**，**红色（常带删除线）= 本版新改动**（删除线 = 取消非常规时段 / 撤回例外）。以「与旧版 PDF 文本 diff」为准，不要只扫一种颜色。
4. **逐课、逐班、逐列**扫色标，尤其注意：
   - **Lecture 日期/时间列**（含括号里的非常规时段）
   - **Classroom / Venue 列**
   - **Tutorial / Session 列**（常在行右侧；A/B 或 C/D 常 **共用一格**）
   - **Exam / Final** 列
5. **色标位置 ≠ 课程代码视觉中心**：Tutorial 色标可能落在两课交界；用「最近的课程代码 + Class」归属。
6. **色标记法常见形态**：
   - `Nov 19 (Thurs) **16 (Mon)** …` → 原周四改到周一（色标是新日期）
   - 日期后色标括号时段 → 该日改为非常规时间；**删除线括号时段** → 取消该非常规时间（回到 dayPattern 常规钟点）
   - Venue 列色标 → 教室变更；例外行消失 → 回到默认教室
7. **与旧版 PDF 逐项 diff**：色标应能在旧版找到对应旧值；新版黑字但与旧版不一致也要同步。
8. 工具辅助：按颜色抽 span + 按 Y 映射到课程行，再人工确认（含删除线）。
